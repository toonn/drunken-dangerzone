...
/**
 * A class that reads a configuration file "servers.xml" and stores the
 * specified servers and port numbers. This class can also return the first free
 * local port that was specified in the config file.
 */
public class ServerConfig {
    ...
    public static int getLocalServerPort() {
        ...
        if (localserverindex > -1)
            return (Integer.valueOf(serverPorts.get(localserverindex)))
                    .intValue();
        else
            return -1;
    }
    ...
    public static int getLocalClientPort() throws IllegalStateException {
        initialize();
        if (localserverindex > -1)
            return (Integer.valueOf(clientPorts.get(localserverindex)))
                    .intValue();
        else
            return -1;
    }
    ...
    public static int getRemoteServerPort(int i) {
        initialize();
        if (i < localserverindex)
            return (Integer.valueOf(serverPorts.get(i))).intValue();
        else
            return (Integer.valueOf(serverPorts.get(i + 1))).intValue();
    }
    ...
    public static int getRemoteClientPort(int i) {
        initialize();
        if (i < localserverindex)
            return (Integer.valueOf(clientPorts.get(i))).intValue();
        else
            return (Integer.valueOf(clientPorts.get(i + 1))).intValue();
    }
    ...
    /**
     * Parse the servers.xml config file. The servers will be stored in the
     * vector hosts, the ports will be stored in the vector ports. The port
     * corresponding to a host will be found in the same position in both
     * vectors, for example the port of the second host is the second element of
     * the ports vector.
     */
    private static void parseConfig() {
        try {
            /**
             * A "Document" an entire XML document. Conceptually, it is the root
             * of the document tree, and provides the primary access to the
             * document's data.
             */
            Document servers;
